...
const Reviews = () => {
  ...
  return (
    <div className=" bg-dark py-4 grid place-items-center">
      <h2 className="text-accent text-center text-[3rem] font-bold">
        What People Are Saying
      </h2>
      <div className="relative flex flex-col items-center justify-center h-[500px] overflow-hidden w-[80%]">
        <div className="relative flex items-center justify-center w-full max-w-5xl">
          {testimonials.map((t, i) => {
            ...
            return (
              <motion.div
                key={t.id}
                className={`absolute w-[300px] h-[300px] rounded-md p-6 text-center shadow-lg [clip-path:polygon(0_0,calc(100%-50px)_0,100%_50px,100%_100%,0_100%)] border-2 border-black mb-[2rem]  ${
              ...
              >
                ...
                />
                <p className="text-lg font-medium leading-snug">"{t.text}"</p>
        ...
        <div className="absolute bottom-6 flex gap-6 ">
          <button
            onClick={prev}
            className="p-3 bg-primary text-white rounded-full hover:bg-primary/80 hover:scale-120 transition"
          >
            <ChevronLeft className="cursor-pointer text-black" />
          </button>
          <button
            onClick={next}
            className="p-3 bg-primary text-white rounded-full hover:bg-primary/80 hover:scale-120 transition"
          >
            <ChevronRight className="cursor-pointer text-black" />
          </button>
        </div>
      </div>
    </div>
